...
- **transcript 監視による Claude の意図の把握**: 多くの競合ツールはフック API（ツール実行やイベント通知）に依存しており、得られる情報は「何のツールが呼ばれたか」に限定される。cc-voice-reporter は transcript .jsonl ファイルを直接監視することで、Claude のテキスト応答（方針説明、判断理由、結果報告など）を含む全出力を読み取り、Claude が何を考え何をしているかを音声で伝えられる。フック API の変更にも影響されにくい独立したアーキテクチャである
