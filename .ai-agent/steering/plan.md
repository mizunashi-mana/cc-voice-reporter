...
フェーズ 3（堅牢化）まで完了。動作確認後の改善・バグ修正を経て、フェーズ 4（機能拡充）に着手中。翻訳機能、定期要約通知、narration 制御などを実装済み。
...
- [x] narration 制御（サマリーモード時の逐次読み上げ無効化）
- [ ] 確認待ちメッセージの順序改善（Issue #34）
- [ ] デフォルトでテキスト中略を無効化（Issue #37）
- [ ] 入力済み時の「入力待ちです」抑制（Issue #38）
- [ ] 多言語対応（日本語・英語）（Issue #24）
...
- [ ] Logger を DI に統一（Issue #47）
- [ ] watcher テストのフレーキー修正（Issue #48）
- [ ] gemma3 モデルでの翻訳アボート問題（Issue #50）
